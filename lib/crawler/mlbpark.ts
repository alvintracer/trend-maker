...
export const mlbparkCrawler: SourceCrawler = {
  async fetchDocuments(url) {
    ...
    if (!response.ok) {
      throw new Error(`MLBPark fetch failed: ${response.status}`);
    }
    ...
    $("#PC_bullpen_today .title a").each((_, element) => {
      ...
      if (!href.includes("m=view")) {
        return;
      }

      const absoluteUrl = href.startsWith("http") ? href : `https://mlbpark.donga.com${href}`;
