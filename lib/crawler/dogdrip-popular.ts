...
function getPageUrls() {
  return Array.from({ length: PAGE_COUNT }, (_, index) => {
    const page = index + 1;
    return `${DOGDRIP_HOST}/dogdrip?sort_index=popular&page=${page}`;
  });
}
...
export const dogdripPopularCrawler: SourceCrawler = {
  async fetchDocuments() {
    ...
    for (const url of getPageUrls()) {
      ...
      if (!response.ok) {
        throw new Error(`Dogdrip popular fetch failed on ${url}: ${response.status}`);
      }

      for (const document of parseDogdripDocuments(await response.text(), "/dogdrip/")) {
    ...
    return {
      ...
      method: "fetch",
      detail: "dogdrip-popular-pages-1-3",
    };
  },
};
